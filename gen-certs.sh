...
keytool -keystore ${server_truststore} -importcert -alias CARoot -file ${out_dir}/ca-cert -noprompt -storepass $password
keytool -keystore ${client_truststore} -importcert -alias CARoot -file ${out_dir}/ca-cert -noprompt -storepass $password
...
keytool -keystore ${server_keystore} -genkeypair -alias $keytool_alias_server -dname "CN=$server_cn, OU=$ou, O=$o, L=$l, ST=$st, C=$c" -noprompt -keypass ${password} -storepass ${password}
...
keytool -keystore ${server_keystore} -certreq -alias $keytool_alias_server -file ${out_dir}/server-cert.csr -noprompt -storepass $password
...
keytool -keystore ${server_keystore} -importcert -alias CARoot -file ${out_dir}/ca-cert -noprompt -storepass $password
keytool -keystore ${server_keystore} -importcert -alias $keytool_alias_server -file ${out_dir}/server-cert-signed -noprompt -storepass $password
...
# Wraps the public key into an X.509 v3 self-signed certificate, which is stored as a single-element certificate chain.
# dname specifies the X.500 Distinguished Name to be associated with alias, and is used as the issuer and subject fields in the self-signed certificate.
keytool -keystore ${client_keystore} -genkeypair -alias $keytool_alias_client -dname "CN=$client_cn, OU=$ou, O=$o, L=$l, ST=$st, C=$c" -validity $validity -noprompt -storepass $password -keypass $password

# Generates client Certificate Signing Request (CSR), using the PKCS#10 format.
# The private key and X.500 Distinguished Name associated with alias are used to create the PKCS#10 certificate request.
keytool -keystore ${client_keystore} -certreq -alias $keytool_alias_client -file ${out_dir}/client-cert.csr -noprompt -storepass $password
...
keytool -keystore ${client_keystore} -importcert -alias CARoot -file ${out_dir}/ca-cert -noprompt -storepass $password
keytool -keystore ${client_keystore} -importcert -alias $keytool_alias_client -file ${out_dir}/client-cert-signed -noprompt -storepass $password
